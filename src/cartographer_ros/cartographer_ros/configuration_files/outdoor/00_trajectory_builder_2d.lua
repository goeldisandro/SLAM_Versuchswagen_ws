...
--Voxel-Filter
  voxel_filter_size = 0.25,	-- fixer Filter: Alle Punkte in einem Voxel werden zu einem zum Voxel-Mittelpunkt zusammengefasst
  adaptive_voxel_filter = {		-- adaptiver Filter
    max_length = 0.9, -- maximale grösse der Voxels
    min_num_points = 80, -- verkleinert voxels bis min-Anzahl RANGE-Punkte enthalten sind. 
    max_range = 60., -- filtert alle Punkte weiter weg
  },
...
-- Festlegen ob ein Scan eingefügt wird oder nicht
  motion_filter = {
    max_time_seconds = 3., -- nach 5 sec wird immer eingefügt
    max_distance_meters = 2, -- wenn zum letzten Scan ein gewisser Abstand festgestellt wird
...
-- Einstellungen zum Erstellen der Submaps
  submaps = {
    num_range_data = 30, --grösse der Submap, so wählen, dass sie in sich konsistent ist
    grid_options_2d = {
