...
--  pure_localization_trimmer = { -- begrenzt die Anzahl Submaps auf 3 und löscht die älteren, um Rechenleistung zu sparen
...
-- frame zuweisungen
  ...
  tracking_frame = "z_up_link", -- wird am besten zu "imu_link" gesetzt, da das IMU-Frame keinen versatzt zum tracking_frame haben darf. 
  published_frame = "base_link", -- ist das child_frame für die tf-Messages des Cartographers, also der Roboter base_link 
  odom_frame = "odom",	-- nötig für provide_odom_frame
  provide_odom_frame = true, 	-- gibd non-loopclosed Position des local SLAM auf dem oben festgelegten Topic. Wird verwendet wenn keine tf-Messages von der Roboter Odometrie versendet werden
  publish_frame_projected_to_2d = true, -- durch die pose extrapolation kann es im 2D-SLAM zu out-off-plane Positionen kommen. So ist sie auf zwei Dimensionen eingeschränkt
-- input data  
  use_odometry = true,
  use_nav_sat = false, -- GPS
  use_landmarks = false,
  num_laser_scans = 0, -- Anzal laser_scan Topics (2D LiDAR, sensor_msgs/LaserScan)
  ...
  num_point_clouds = 1, -- num point_cloud topics (3D LiDAR, sensor_msgs/PointCloud2)
