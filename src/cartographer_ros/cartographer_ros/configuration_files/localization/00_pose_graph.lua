POSE_GRAPH = {
  optimize_every_n_nodes = 0,		-- set to 0 for testing local slam (global slam is run every n nodes)
  
-- Sucht nach Constraints
  constraint_builder = {
    fast_correlative_scan_matcher = {	-- 2D Sucht passensde Maps (Submaps - Nodes) -> Constraint
      linear_search_window = 0.1,	-- je kleiner desto schneller
      angular_search_window = math.rad(.01),
    ...
    sampling_ratio = 0.000000001, --A constraint will be added if the proportion of added constraints to potential constraints dropsbelow this number
    max_constraint_distance = 0.01, --max Abstand von einem Node zu einer Submap
    min_score = 0.999999,	-- min Score des FastCorrelativScanMatchers
...
-- optimiert den Pose Graph anhand aller constraints
  ...
  optimization_problem = {
    ...
    local_slam_pose_translation_weight = 1e1,	--of the local slam pose
  ...
  max_num_final_iterations = 1,
  global_sampling_ratio = 1, -- Rate at which we sample a single trajectory’s nodes for global localization.
  log_residual_histograms = true,
  global_constraint_search_after_n_seconds = 1000.,
}
